...
int doubleBranchCount(Node *node)
{
    ...
    if (node->left != NULL && node->right != NULL)
        return doubleBranchCount(node->left) + doubleBranchCount(node->right) + 1;
         // 如果节点有两个孩子，返回左子树和右子树的双分支节点个数之和，再加1
    else
        return doubleBranchCount(node->left) + doubleBranchCount(node->right); 
        // 如果节点只有一个孩子或没有孩子，返回左子树和右子树的双分支节点个数之和
}
